...
export const addGains = addCosts;
...
export const totalGain = totalCost;
...
export const toGainPercentages = toPercentages;
...
export const annualValue = annualCost;
...
// Brain state reflects NET threat load: cost accumulation minus what voice built back.
// All-Voice play stays prefrontal active; all-F/F/F tips toward amygdala dominance.
export function brainState(costs, gains = EMPTY_GAINS) {
  const net = totalCost(costs) - totalGain(gains);
  if (net < 50) return { label: "PREFRONTAL ACTIVE", color: "#AAC785" };
  if (net < 100) return { label: "SHIFTING TO THREAT", color: "#EB865C" };
